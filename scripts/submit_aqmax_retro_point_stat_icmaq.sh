...
while [ ${NOW} -le ${LASTDAY} ]; do
    ...
    if [ -s ${fcst_dir}/aqm.${PDYm3}/aqm.t06z.ave_24hr_pm25_bc.227.grib2 ] || [ -s ${fcst_dir}/aqm.${PDYm3}/aqm.t12z.ave_24hr_pm25_bc.227.grib2 ]; then
        ...
        if [ -s ${fcst_dir}/aqm.${PDYm3}/aqm.t06z.ave_24hr_pm25_bc.227.grib2 ] || [ -s ${fcst_dir}/aqm.${PDYm3}/aqm.t12z.ave_24hr_pm25_bc.227.grib2 ]; then
            ...
            chkfile=aqm.${PDYm3}/aqm.t06z.ave_24hr_pm25_bc.227.grib2
            if [ ! -s ${fcst_dir}/${chkfile} ]; then
                echo "Can not find ${chkfile} in ${FCST_INPUT_NCO} and ${FCST_INPUT_USER}, skip to next day"
            fi
            chkfile=aqm.${PDYm3}/aqm.t12z.ave_24hr_pm25_bc.227.grib2
            if [ ! -s ${fcst_dir}/${chkfile} ]; then
                echo "Can not find ${chkfile} in ${FCST_INPUT_NCO} and ${FCST_INPUT_USER}, skip to next day"
            fi
            if [ 1 -eq 1 ]; then
