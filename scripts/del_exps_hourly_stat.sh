...
while [ ${NOW} -le ${LASTDAY} ]; do
    ...
    for i in "${hr_type[@]}"; do
        for j in "${case[@]}"; do
	    echo "${aq_pb2nc}/${j}/${i}"
            /bin/rm -f  ${aq_pb2nc}/${j}/${i}/*${NOW}*
	    echo "${aq_hourly}/${i}/${j}"
            /bin/rm -f  ${aq_hourly}/${i}/${j}/*${NOW}*
        done
    done
    i=aqmmax
    j=aqmmax
    for k in "${case[@]}"; do
        ...
        /bin/rm -f  ${aqmmax_hourly}/${i}/stat/${j}/${k}/*${NOW}*
            echo "${aqmmax_hourly}/aqmax1/${k}"
        /bin/rm -f  ${aqmmax_hourly}/aqmax1/${k}/*${NOW}*
	    echo "${aqmmax_hourly}/aqmax8/${k}"
        /bin/rm -f  ${aqmmax_hourly}/aqmax8/${k}/*${NOW}*
    done
    i=pmmax
    j=pmmax
    for k in "${case[@]}"; do
        ...
        /bin/rm -f  ${aqmmax_hourly}/${i}/${j}/${k}/*${NOW}*
    done
    j=pmave
    for k in "${case[@]}"; do
	    echo "${aqmmax_hourly}/${i}/stat/${j}"
        /bin/rm -f  ${aqmmax_hourly}/${i}/stat/${j}/${k}/*${NOW}* 
	    echo "${aqmmax_hourly}/${i}/${j}/${k}"
        /bin/rm -f  ${aqmmax_hourly}/${i}/${j}/${k}/*${NOW}* 
    done
