...
while [ ${NOW} -le ${LASTDAY} ]; do
    ...
    out_logfile=${logdir}/${jjob}.icmaq.log
    err_logfile=${logdir}/${jjob}.icmaq.log
    ...
    OBS_INPUT_USER=/lfs/h2/emc/physics/noscrub/Perry.Shafran/com/hourly/prod
    ...
    FCST_INPUT_USER=/lfs/h2/emc/ptmp/${USER}/com/aqm/${EXP}
    fcst_dir=${FCST_INPUT_NCO}
    ## if [ -s ${fcst_dir}/cs.${PDYm3}/postprd/rrfs.t06z.natlevf072.tm00.grib2 ] || [ -s ${fcst_dir}/cs.${PDYm3}/postprd/rrfs.t12z.natlevf072.tm00.grib2 ]; then
    if [ -s ${fcst_dir}/cs.${PDYm3}/rrfs.t12z.natlevf072.tm00.grib2 ]; then
        fcst_select=${fcst_dir}
    else
        fcst_dir=${FCST_INPUT_USER}
        ## if [ -s ${fcst_dir}/cs.${PDYm3}/postprd/rrfs.t06z.natlevf072.tm00.grib2 ] || [ -s ${fcst_dir}/cs.${PDYm3}/postprd/rrfs.t12z.natlevf072.tm00.grib2 ]; then
        if [ -s ${fcst_dir}/cs.${PDYm3}/postprd/rrfs.t12z.natlevf072.tm00.grib2 ]; then
            fcst_select=${fcst_dir}
        else
            ## chkfile=cs.${PDYm3}/aqm.t06z.natlevf072.tm00.grib2
            ## if [ ! -s ${fcst_dir}/${chkfile} ]; then
            ##     echo "Can not find ${chkfile} in ${FCST_INPUT_NCO} and ${FCST_INPUT_USER}, skip to next day"
            ## fi
            chkfile=cs.${PDYm3}/aqm.t12z.natlevf072.tm00.grib2
            if [ ! -s ${fcst_dir}/${chkfile} ]; then
                echo "Can not find ${chkfile} in ${FCST_INPUT_NCO} and ${FCST_INPUT_USER}, skip to next day"
            fi
