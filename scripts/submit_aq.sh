...
while [ ${NOW} -le ${LASTDAY} ]; do
    ...
    if [ "${run_type}" == "retro" ]; then
       fcst_dir=/gpfs/dell2/emc/modeling/noscrub/${USER}/verification/aqm/prod
        if [ -s ${fcst_dir}/aqm.${PDYm3}/aqm.t12z.awpozcon.f72.148.grib2 ]; then
            run_type=retro
        else
            echo "Can not find model output data ${PDYm3} in ${fcst_dir} and /gpfs/hps/nco/ops/com/aqm/prod, program stop"
            exit
        fi
    fi
    run_script=run_${jjob}.sh
    sed -e "s!xxRUN_TYPE!${run_type}!" -e "s!xxENVIR!${envir}!" -e "s!xxJOB!${jjob}!" -e "s!xxOUTLOG!${out_logfile}!" -e "s!xxERRLOG!${err_logfile}!" -e "s!xxDATEp1!${PDYp1}!" -e "s!xxDATE!${NOW}!" ${script_dir}/${script_base} > ${working_dir}/${run_script}
